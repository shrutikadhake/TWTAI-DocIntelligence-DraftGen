...
function loadHtml(filePath?: string, html?: string): { html: string; label: string } {
  if (html && html.trim().length > 0) return { html, label: "(inline HTML)" };
...
function formatIssues(title: string, label: string, issues: Issue[]): string {
  ...
  const out: string[] = [];
  out.push(title, `Document: ${label}`,
    `Result: ${e.length} error(s), ${w.length} warning(s), ${n.length} note(s)`, "");
...
function scoreFromIssues(issues: Issue[]): number {
  const penalty = issues.reduce((a, i) =>
    a + (i.severity === "error" ? 15 : i.severity === "warning" ? 5 : 1), 0);
  return Math.max(0, 100 - penalty);
}

/* ---------------- Writing-style rules ---------------- */

type Guide = "microsoft" | "ibm" | "google";

interface TermRule { re: RegExp; severity: Issue["severity"]; msg: string; }

/**
 * Rules shared by ALL three guides (they agree on these).
 * TO ADD YOUR OWN RULE: copy a line, change the pattern and the message.
 */
const SHARED_RULES: TermRule[] = [
...
const GUIDE_RULES: Record<Guide, TermRule[]> = {
  microsoft: [
...
function checkWritingStyle(html: string, guide: Guide): Issue[] {
  const issues: Issue[] = [];
...
function checkGeneralRules(html: string): Issue[] {
  const issues: Issue[] = [];
  const cleaned = stripNonContent(html);

  // (a) Conversion leftovers — signs a JSON field failed to render.
  const artifacts: TermRule[] = [
...
function checkStructure(html: string): Issue[] {
  const issues: Issue[] = [];
  const cleaned = stripNonContent(html);
  ...
  while ((t = tagRe.exec(cleaned)) !== null) {
    ...
    if (!closing) { stack.push({ name, index: t.index }); continue; }
    if (stack.length && stack[stack.length - 1].name === name) { stack.pop(); continue; }
  ...
  while ((im = idRe.exec(cleaned)) !== null) {
    ...
    else seen.set(im[1], lineOf(cleaned, im.index));
  }
  return issues;
}
...
server.resource(
  "styleguide",
  "styleguide://main",
  { description: "The full documentation style guide (Microsoft, IBM, Google, and general rules). Read this when doing a judgment-based review of tone, clarity, and structure that the mechanical tools can't measure.", mimeType: "text/markdown" },
  async (uri) => ({
...
server.tool(
  ...
  "Checks an HTML document for well-formedness: unclosed or mismatched tags, duplicate IDs, missing DOCTYPE and <title>.",
  inputSchema,
  async ({ file_path, html }) => {
    const { html: doc, label } = loadHtml(file_path, html);
...
server.tool(
  "check_general_rules",
  "Checks rules that apply regardless of style guide: leftover conversion artifacts (literal 'undefined'/'null'/'[object Object]', unresolved {{placeholders}}, TODO markers, lorem ipsum), empty headings/paragraphs/cells/links, and semantic-HTML5 usage. Run this on every file produced by JSON-to-HTML conversion.",
  inputSchema,
  async ({ file_path, html }) => {
    const { html: doc, label } = loadHtml(file_path, html);
...
server.tool(
  "run_quality_report",
  "Runs all three checks (writing style for the selected guide, HTML structure, general documentation rules) and returns one combined report with a 0-100 score per area and an overall grade. style_guide defaults to 'microsoft'.",
  inputSchema,
  async ({ file_path, html, style_guide }) => {
    const guide: Guide = style_guide ?? "microsoft";
    const { html: doc, label } = loadHtml(file_path, html);
    ...
    const out: string[] = ["DOC QUALITY REPORT", `Document: ${label}`, `Style guide: ${guide}`, `Overall: ${overall}/100 (Grade ${grade})`, ""];
    for (const a of scored) {
      out.push(`── ${a.name}: ${a.score}/100 ──`);
      if (a.issues.length === 0) out.push("  ✔ No issues.");
